...
# 这个脚本的目的是将 PDDL 文件的内容转换为一个内部模型字典，这个字典可以被用来进一步处理或分析规划问题。
...
def store_actions(reader):
    ...
    for act in reader.problem.actions.values():
        ...
        action_model[act.name][PARARMETERS] = [(p.symbol, p.sort.name) for p in act.parameters]
...
#     parse_model('pr-domain_elevator.pddl','pr-problem.pddl')

# 用于测试
if __name__ == '__main__':
    model = parse_model('domain_elevator.pddl','problem.pddl')
    print(model)
