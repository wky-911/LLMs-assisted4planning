...
#     )
#     print(response)
#     raw_output = response['output']['text'] # qwen-max\qwen-plus\qwen-turbo
#     # raw_output = response['output']['choices'][0]['message']['content'] # deepseek-r1
...
#     domain_description = domain_data.get('blocksworld', '')
#
...
#         node = root
#         applied_actions = []
#         while node.children and node.is_fully_expanded(applicable_actions(node.state, domain, obj)):
#             node = node.best_child()  # 定位到最佳路径上还未完全拓展完的节点
#             applied_actions.append(node.action) # 生成动作轨迹
#
#         if goal_achieved(node.state, goal):
#             print("goal achieved!!!")
#             break
#
#         # 提示词由领域描述、初始状态、当前状态、目标状态、动作轨迹、可执行动作序列构成
#         prompt = create_prompt(domain_description, init_state, node.state, goal, applied_actions, applicable_actions(node.state, domain, obj))
#         # print(f"applied_actions: {applied_actions}")
#         # print(f"applicable: {[(action.action_name, action.parameters) for action in applicable_actions(node.state, domain, obj)]}")
#         # llm_list = [((action1_name, action1_params), reward1), ((action2_name, action2_params), reward2),...]
#         llm_list = ask_qwen(str(prompt))
#
#         # print(f"prompt: {prompt}")
#         times += 1
#         # print(f"node: {node}")
#         print(f"applicable actions: {[(action.action_name, action.parameters) for action in applicable_actions(node.state, domain, obj)]}")
...
#     problem_file = "blocksworld/problem_simple.pddl"
...
#         print(step)
#
#     with open("result_bw.txt", "w") as file:
#         for step in plan:
#             file.write(f"({step})\n")
