...
from components.llm_logistics import *


"""
1.调用DW本身规划总实例，时间限定10s  √
------------------------------------------
2.拆解目标状态，每个子实例的规划时间限定在30s  √
-------------------------------------------
3.对于子实例，如果超过30s，调用LLM&MCTS执行5个步骤的跳跃  √
4.得到新状态后调用DW，规划时间限定在30s  √
-------------------------------------------
5.如果失败则调用LLM将子实例进行再拆分，限定规划时间在30s  √
...
def write_instance_to_remote(instance_content, directory='logistics'):
    """
...
def ask4decompose(prompt4usr, domain_name='logistics'):
...
def solve_sub_goals_dw_v3(problem_file, init_state, goal_state, domain,
    ...
    ins_list = get_ins_list(goal_state)  # 构建子目标实例列表
...
if __name__ == '__main__':
    ...
    for index in range(1, 85):
        ...
        ins_list = get_ins_list(goal) # 子目标序列表
        print(ins_list)
